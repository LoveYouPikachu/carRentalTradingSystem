...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate', 'layedit'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '${mao}/news/loadAllNews.action' //数据接口
        ...
        $("#doSearch").click(function () {
            ...
            //alert(params);
            tableIns.reload({
                url: "${mao}/news/loadAllNews.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(newsTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.title + '】这个公告么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${mao}/news/deleteNews.action", {id: data.id}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openUpdateNews(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${mao}/news/updateNews.action";
                }
            });
        }
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些公告么？', function (index) {
                //向服务端发送删除指令
                $.post("${mao}/news/deleteBatchNews.action", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
