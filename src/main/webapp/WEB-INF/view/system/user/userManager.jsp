...
<head>
    ...
    <link rel="stylesheet" href="${mao}/static/layui_ext/dtree/dtree.css">
    <link rel="stylesheet" href="${mao}/static/layui_ext/dtree/font/dtreefont.css">
</head>
...
<script type="text/javascript">
    var tableIns;
    layui.extend({
        dtree: '${mao}/static/layui_ext/dist/dtree'
        ...
        tableIns = table.render({
            ...
            , url: '${mao}/user/loadAllUser.action' //数据接口
        ...
        $("#doSearch").click(function () {
            ...
            //alert(params);
            tableIns.reload({
                url: "${mao}/user/loadAllUser.action?" + params,
                page:{curr:1}
            })
        });
        ...
        table.on('tool(userTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.realname + '】这个用户么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${mao}/user/deleteUser.action", {userid: data.userid}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
                ...
                layer.confirm('真的重置【' + data.realname + '】这个用户的密码吗？', function (index) {
                    //向服务端发送重置密码
                    $.post("${mao}/user/resetUserPwd.action", {userid: data.userid}, function (res) {
                        layer.msg(res.msg);
                    })
                });
            }
        ...
        function openUpdateUser(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${mao}/user/updateUser.action";
                }
            });
        }
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些用户么？', function (index) {
                //向服务端发送删除指令
                $.post("${mao}/user/deleteBatchUser.action",params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
        ...
        function openselectUserRole(data) {
            mainIndex=layer.open({
                ...
                yes:function (index, layero) {
                    ...
                    //保存
                    $.post("${mao}/user/saveUserRole.action",params,function (obj) {
                        layer.msg(obj.msg);
                        //关闭弹出层
                        layer.close(mainIndex);
                    })
                ...
                success:function (index) {
                    ...
                    var roleTableIns = table.render({
                        ...
                        , url: '${mao}/user/initUserRole.action?userid='+data.userid //数据接口
                        , title: '角色列表'//数据导出来的标题
                        , cellMinWidth: 100 //设置列的最小默认宽度
                        , cols: [[   //列表数据
                            {type: 'checkbox', fixed: 'left'}
