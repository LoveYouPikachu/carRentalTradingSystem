...
<script src="${mao}/static/layui_ext/dist/dtree.js"></script>
<script type="text/javascript">
    ...
    layui.extend({
        ...
        tableIns = table.render({
            ...
            , url: '${mao}/menu/loadAllMenu.action' //数据接口
        ...
        $("#doSearch").click(function () {
            ...
            //alert(params);
            tableIns.reload({
                url: "${mao}/menu/loadAllMenu.action?" + params,
                page:{curr:1}
            })
        });
        ...
        table.on('tool(menuTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                //判断当前菜单有没有子节点
                $.post("${mao}/menu/checkMenuHasChildren.action?id=" + data.id, function (obj) {
                    if (obj.code >= 0) {
                        layer.msg("当前菜单有子节点，请先删除子节点");
                    } else {
                        layer.confirm('真的删除【' + data.title + '】这个菜单么？', function (index) {
                            //向服务端发送删除指令
                            $.post("${mao}/menu/deleteMenu.action", {id: data.id}, function (res) {
                                layer.msg(res.msg);
                                //刷新数据表格
                                tableIns.reload();
                                //刷新左边的数
        ...
        function openUpdateMenu(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${mao}/menu/updateMenu.action";
                    //反选下拉树
                    var pid = data.pid;
        ...
        var menuTree = dtree.render({
            ...
            url: "${mao}/menu/loadMenuManagerLeftTreeJson.action?spread=1",  // 使用url加载（可与data加载同时存在）
            icon: "2",
            accordion: true //能否下拉
        });
    ...
    function reloadTable(id) {
        tableIns.reload({
            url: "${mao}/menu/loadAllMenu.action?id=" + id
        })
    }
</script>
</body>
