...
<head>
    ...
    <link rel="stylesheet" href="${mao}/static/layui/css/layui.css" media="all"/>
    <link rel="stylesheet" href="${mao}/static/css/public.css" media="all"/>
</head>
...
<script src="${mao}/static/layui/layui.js"></script>
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'upload'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '${mao}/car/loadAllCar.action' //数据接口
            ...
            , cols: [[   //列表数据
                ...
                , {
                    field: 'carimg', title: '缩略图', align: 'center', width: '80', templet: function (d) {
                        return "<img width=40 height=40 src=${mao}/file/downloadShowFile.action?path=" + d.carimg + "/>";
                    }
                }
...
//            alert(params);
            tableIns.reload({
                url: "${mao}/car/loadAllCar.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(carTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.carnumber + '】这个车辆么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${mao}/car/deleteCar.action", {carnumber: data.carnumber}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddCar() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#dataFrm")[0].reset();
                    //设置默认图片
                    $("#showCarImg").attr("src", "${mao}/file/downloadShowFile.action?path=images/defaultcarimage.jpg");
                    $("#carimg").val("images/defaultcarimage.jpg");
                    url = "${mao}/car/addCar.action";
        ...
        function openUpdateCar(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#showCarImg").attr("src", "${mao}/file/downloadShowFile.action?path=" + data.carimg);
                    url = "${mao}/car/updateCar.action";
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些车辆么？', function (index) {
                //向服务端发送删除指令
                $.post("${mao}/car/deleteBatchCar.action", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
        ...
        upload.render({
            elem: '#carimgDiv',
            url: '${mao}/file/uploadFile.action',
            method: "post",  //此处是为了演示之用，实际使用中请将此删除，默认用post方式提交
            acceptMime: 'images/*',
            field: "mf",
            done: function (res, index, upload) {
                $('#showCarImg').attr('src', "${mao}/file/downloadShowFile.action?path=" + res.data.src);
        ...
        function showCarImage(data) {
            mainIndex = layer.open({
                ...
                area: ['750px', '500px'],
                success: function (index) {
                    $("#view_carimg").attr("src","${mao}/file/downloadShowFile.action?path="+data.carimg);
                }
            });
        }
